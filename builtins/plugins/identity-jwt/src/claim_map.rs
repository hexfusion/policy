...
impl ClaimMapper for StandardClaimMap {
    ...
    fn map_workload(&self, claims: &ClaimMap) -> Option<WorkloadIdentity> {
        ...
        // Every other claim → `caller_workload.claim.<name>`. A JWT-SVID
        // carries the issuer's authorization decision alongside the
        // identity; dropping it here would leave a policy able to say
        // which peer is calling but not what that peer was granted.
        // Reserved: the claims already mapped to structured fields plus
        // the JWT registered claims, matching `map_subject`.
